...
import { PRIORITY_COLORS, STATUS_LABELS, PRIORITY_LABELS, timeAgo } from '../lib/helpers'
...
function UserDot({ lat, lng }) {
  ...
  return (
    <CircleMarker center={[lat, lng]} radius={8} pathOptions={{ color: '#3b82f6', fillColor: '#3b82f6', fillOpacity: 1, weight: 3 }}>
...
export default function IncidentMap({ incidents, center, height = 500, onSelect, userLocation }) {
      ...
      <MapContainer center={defaultCenter} zoom={13} style={{ height: '100%', width: '100%' }} scrollWheelZoom>
        ...
        {incidents.map((inc) => (
          <Marker
            ...
            icon={makeIcon(inc.severity || inc.ai_priority, inc.status === 'reported' ? 18 : 22)}
            ...
            <Popup>
              <div style={{ minWidth: 200 }}>
                <div style={{ fontWeight: 700, color: '#ef4444', marginBottom: 4 }}>
                  ...
                  <span className={`badge badge-priority-${inc.severity || inc.ai_priority}`} style={{ marginLeft: 8 }}>
                    {PRIORITY_LABELS[inc.severity || inc.ai_priority] || ''}
                  </span>
                </div>
                <div style={{ fontSize: 13, marginBottom: 6 }}>{inc.ai_summary || inc.description?.slice(0,120)}</div>
                <div style={{ fontSize: 11, color: '#9ca3af', marginBottom: 8 }}>
                  {STATUS_LABELS[inc.status]} · {timeAgo(inc.created_at)}
                </div>
                <button className="btn btn-primary btn-sm" onClick={() => navigate(`/incidents/${inc.id}`)}>
                  View Details →
                </button>
              </div>
            </Popup>
