...
function PickPin() {
  // Premium red teardrop with an exclamation-mark "pick here" icon, matches the main Map.
  const html = `<div class="pin-drop" style="position:relative;width:40px;height:54px;">
    <svg viewBox="0 0 40 54" width="40" height="54" xmlns="http://www.w3.org/2000/svg">
      <defs><filter id="rpsh" x="-40%" y="-40%" width="180%" height="200%"><feDropShadow dx="0" dy="3" stdDeviation="2.5" flood-color="#0f172a" flood-opacity=".32"/></filter></defs>
      <path filter="url(#rpsh)" d="M20 2C9.7 2 1.5 10 1.5 20c0 13 18.5 32 18.5 32S38.5 33 38.5 20C38.5 10 30.3 2 20 2z" fill="#ef4444" stroke="#fff" stroke-width="2"/>
      <circle cx="20" cy="19" r="10" fill="rgba(255,255,255,0.2)"/>
      <g transform="translate(12,11)" fill="#fff">
        <circle cx="8" cy="3" r="1.8"/><rect x="7" y="6" width="2" height="8" rx="1"/>
      </g>
    </svg>
  </div>`;
  return L.divIcon({
...
export default function Report() {
  ...
  const zoomIn = () => mapInst?.zoomIn();
  const zoomOut = () => mapInst?.zoomOut();
  const centerOnMe = () => {
    setLocating(true);
    setMapReady(false);
    getLocation()
  ...
  useEffect(() => {
    ...
    getLocation()
      .then(setPos)
      .catch(() => setPos({ lat: 27.7172, lng: 85.324 }))
      .finally(() => setLocating(false));
  }, []);
  ...
  const simulateVoice = () => {
    ...
    if (recording) {
      ...
      const samples = {
        flood:
          "Help, flood water is rising, we are trapped on the roof with children.",
        fire: "Fire has broken out in the building, smoke everywhere, people are screaming.",
        medical:
          "There's been an accident, someone is badly injured and bleeding, please send help.",
        accident:
          "Two cars crashed badly on the main road, people are trapped inside.",
        default:
          "Please send help immediately, this is an emergency situation.",
      };
  ...
  const submit = async () => {
    ...
    if (!desc.trim() && !voiceTranscript) {
      toast(t("report.need_desc"), "err");
      return;
    }
    if (!pos) {
      toast(t("report.need_loc"), "err");
      return;
    }
    setSubmitting(true);
    try {
      ...
      const { data } = await api.post("/api/incidents", {
        ...
        address,
        photo_urls: images.length
          ? images.map((_, i) => `upload_${i + 1}.jpg`)
          : undefined,
  ...
  if (step === 4 && result)
    return <ResultCard result={result} onClose={() => nav("/app")} />;
    ...
    <div className="max-w-4xl mx-auto p-4 md:p-6">
      ...
      <div className="mb-6 mt-2">
        <h1 className="text-2xl md:text-3xl font-extrabold flex items-center gap-2">
          <AlertTriangle className="text-red-500" /> Report Emergency
        </h1>
        <p className="text-ink-500 dark:text-ink-400 text-sm mt-1">
          Share what's happening — AI will triage instantly.
        </p>
      </div>
      ...
      <div className="flex items-center gap-2 mb-6">
        {STEPS.map((s, i) => (
          <React.Fragment key={s}>
            <button
              onClick={() => setStep(i)}
              className={`flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-semibold transition ${i === step ? "bg-brand-600 text-white shadow-lg shadow-brand-600/30" : i < step ? "bg-emerald-100 text-emerald-700 dark:bg-emerald-500/20 dark:text-emerald-300" : "bg-ink-100 dark:bg-ink-800 text-ink-500"}`}
            >
              {i < step ? <CheckCircle2 size={14} /> : i + 1} {s}
            </button>
            {i < STEPS.length - 1 && (
              <div
                className={`flex-1 h-0.5 ${i < step ? "bg-emerald-500" : "bg-ink-200 dark:bg-ink-800"}`}
              />
            )}
          </React.Fragment>
        ))}
        ...
        <AnimatePresence mode="wait">
          ...
          >
            {step === 0 && (
              <div className="space-y-5">
                ...
                <div>
                  ...
                  <Textarea
                    rows={5}
                    placeholder="Describe what you see, any injuries, the location, urgency..."
                    value={desc}
                ...
                <div className="grid md:grid-cols-3 gap-3">
                  ...
                  <div>
                    <label className="text-sm font-semibold mb-2 block flex items-center gap-1">
                      <Phone size={14} /> Contact (optional)
                    </label>
                    <Input
              ...
              <div className="space-y-5">
                <div>
                  <label className="text-sm font-semibold mb-2 block flex items-center gap-2">
                    <ImageIcon size={16} /> Upload photos/videos (optional — AI
                    will analyze them)
                  </label>
                  <input
                    ref={fileRef}
                  ...
                  <button
                    ...
                    className="w-full p-6 rounded-xl border-2 border-dashed border-ink-300 dark:border-ink-700 hover:border-brand-500 transition flex flex-col items-center gap-2 text-ink-500"
                  >
                    <Upload size={28} />
                    ...
                    <span className="text-xs">
                      AI will detect fire, smoke, flood, damage, injuries
                    </span>
                  </button>
                  {images.length > 0 && (
                    <div className="mt-3 grid grid-cols-4 gap-2">
                      {images.map((src, i) => (
                        <div
                          key={i}
                          className="relative aspect-square rounded-lg overflow-hidden bg-ink-100 dark:bg-ink-800"
                        >
                          <img
                            src={src}
                            alt=""
                ...
                <div>
                  <label className="text-sm font-semibold mb-2 block flex items-center gap-2">
                    <Mic size={16} /> Record voice note (optional —
                    auto-transcribed)
                  </label>
                  <button
                  ...
                  >
                    <div
                      className={`w-12 h-12 rounded-full grid place-items-center ${recording ? "bg-red-500 text-white" : "bg-ink-100 dark:bg-ink-800"}`}
                    >
                      {recording ? (
                        <span className="text-sm font-bold">{recTimer}s</span>
                      ) : (
                        <Mic size={22} />
                      )}
                    </div>
                    <div className="text-left">
                      ...
                      <div className="text-xs text-ink-500">
                        Speech-to-text · Language detection · Auto-translate
                      </div>
                    </div>
                  </button>
                  {voiceTranscript && (
            ...
            {step === 2 && (
              <div>
                <label className="text-sm font-semibold mb-2 block flex items-center gap-2">
                  <MapPin size={16} /> Pin the exact location{" "}
                  ...
                  {mapReady && (
                    <div className="absolute right-3 bottom-3 z-[400] flex flex-col gap-1.5">
                      <div className="flex flex-col rounded-xl overflow-hidden bg-white/95 dark:bg-ink-900/95 backdrop-blur border border-ink-200/80 dark:border-ink-700/80 shadow-lg">
                        <button
                          onClick={zoomIn}
                          className="h-9 w-9 grid place-items-center hover:bg-ink-50 dark:hover:bg-ink-800 border-b border-ink-100 dark:border-ink-800 text-ink-700 dark:text-ink-200"
                        >
                          <Plus size={16} />
                        </button>
                        <button
                          onClick={zoomOut}
                          className="h-9 w-9 grid place-items-center hover:bg-ink-50 dark:hover:bg-ink-800 text-ink-700 dark:text-ink-200"
                        >
                          <Minus size={16} />
                        </button>
                      </div>
                      <button
                        onClick={centerOnMe}
                        className="h-9 w-9 grid place-items-center rounded-xl bg-white/95 dark:bg-ink-900/95 backdrop-blur border border-ink-200/80 dark:border-ink-700/80 shadow-lg hover:bg-blue-50 dark:hover:bg-blue-500/10 text-blue-600 dark:text-blue-400"
                      >
                        {locating ? (
                ...
                <div className="flex items-center justify-between mt-3 text-xs text-ink-500 gap-2 flex-wrap">
                  <span>
                    Click on the map to place pin · Drag to pan · Scroll to zoom
                  </span>
                </div>
                {pos && (
                  <div className="mt-2 text-xs font-mono text-ink-400">
            ...
            {step === 3 && (
              <div className="space-y-4">
                <div className="p-4 rounded-xl bg-gradient-to-br from-purple-50 to-blue-50 dark:from-purple-500/10 dark:to-blue-500/10 border border-purple-200 dark:border-purple-500/20 flex items-start gap-3">
                  ...
                  <div>
                    <div className="font-bold text-sm text-purple-900 dark:text-purple-200">
                      Ready to submit — AI will analyze instantly
                    </div>
                    <div className="text-xs text-ink-600 dark:text-ink-400 mt-1">
                      After submission, our AI verifies, summarizes,
                      prioritizes, checks for duplicates, and dispatches to
                      nearest responders.
                    </div>
                  </div>
                </div>
                <div className="grid md:grid-cols-2 gap-3 text-sm">
                  <Review label="Type">
                    {EMERGENCY_CATEGORIES.find((c) => c.id === cat)?.emoji}{" "}
                    {EMERGENCY_CATEGORIES.find((c) => c.id === cat)?.label}
                  </Review>
                  ...
                  <Review label="Voice note">
                    {voiceTranscript ? "Yes" : "None"}
                  </Review>
                  <Review label="Location" span2>
                    {pos
                      ? `${pos.lat.toFixed(4)}, ${pos.lng.toFixed(4)}`
                      : "Not set"}
                  </Review>
                  <Review label="Description" span2>
                    {desc || voiceTranscript}
                  </Review>
                </div>
              </div>
          ...
          <div className="flex gap-2">
            {step > 0 && (
              <Button
                ...
                disabled={submitting}
              >
                <ChevronLeft size={16} /> {t("report.back")}
              </Button>
            )}
            {step < 3 && (
              <Button
                onClick={() => canNext && !submitting && setStep((s) => s + 1)}
                disabled={!canNext || submitting}
              >
                {t("report.next")} <ChevronRight size={16} />
              </Button>
            )}
            {step === 3 && (
              <Button onClick={submit} disabled={submitting || !pos}>
                {submitting ? (
                  <>
                  ...
                  <>
                    <Send size={16} /> {t("report.submit_ai")}
                  </>
                )}
              </Button>
            )}
...
function Review({ label, children, span2 }) {
  return (
    <div
      className={`p-3 rounded-xl bg-ink-50 dark:bg-ink-800/50 ${span2 ? "md:col-span-2" : ""}`}
    >
      ...
      >
        <Card className="!p-8 text-center">
          ...
          <p className="text-ink-500 mt-2">
            AI has analyzed your report and responders have been notified.
          </p>
                  ...
                  color={
                    {
                      ...
                      moderate: "amber",
                      low: "green",
                    }[result.ai_severity] || "slate"
                  }
                >
                  {result.ai_severity?.toUpperCase()}
                </Badge>
              </Mini>
              <Mini label="Confidence">
            ...
            {result.ai_required_resources?.length > 0 && (
              <div>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {result.ai_required_resources.map((r) => (
          ...
          <div className="mt-6 flex gap-2 justify-center">
            <Button
              onClick={() =>
                window.open(`/app/incidents/${result.id}`, "_blank")
              }
            >
              Track Status →
            </Button>
...
function Mini({ label, children }) {
  return (
    <div className="p-2 rounded-lg bg-ink-50 dark:bg-ink-800/50 text-center">
      <div className="text-[10px] font-bold text-ink-500 uppercase">
        {label}
      </div>
      <div className="mt-1 text-sm font-bold flex items-center justify-center gap-1">
        {children}
      </div>
    </div>
  );
