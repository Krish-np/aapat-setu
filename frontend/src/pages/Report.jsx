...
function PickPin() {
  ...
  return L.divIcon({ className:'custom-pin', html, iconSize:[40,54], iconAnchor:[20,54], popupAnchor:[0,-54] })
}
...
export default function Report() {
  ...
  const centerOnMe = () => {
    setLocating(true); setMapReady(false)
    getLocation().then(setPos).finally(()=>setLocating(false))
  }
                  ...
                  {mapReady && (
                    <div className="absolute right-3 bottom-3 z-[400] flex flex-col gap-1.5">
                      <div className="flex flex-col rounded-xl overflow-hidden bg-white/95 dark:bg-ink-900/95 backdrop-blur border border-ink-200/80 dark:border-ink-700/80 shadow-lg">
                        <button onClick={zoomIn} className="h-9 w-9 grid place-items-center hover:bg-ink-50 dark:hover:bg-ink-800 border-b border-ink-100 dark:border-ink-800 text-ink-700 dark:text-ink-200"><Plus size={16}/></button>
                        <button onClick={zoomOut} className="h-9 w-9 grid place-items-center hover:bg-ink-50 dark:hover:bg-ink-800 text-ink-700 dark:text-ink-200"><Minus size={16}/></button>
                      </div>
                      <button onClick={centerOnMe} className="h-9 w-9 grid place-items-center rounded-xl bg-white/95 dark:bg-ink-900/95 backdrop-blur border border-ink-200/80 dark:border-ink-700/80 shadow-lg hover:bg-blue-50 dark:hover:bg-blue-500/10 text-blue-600 dark:text-blue-400">
