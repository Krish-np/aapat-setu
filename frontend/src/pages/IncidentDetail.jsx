...
export default function IncidentDetail() {
  ...
  const load = () =>
    Promise.all([api.get(`/api/incidents/${id}`), api.get("/api/tasks")]).then(
      ([i, t]) => {
        setInc(i.data);
        setTasks(t.data.filter((x) => x.incident_id === Number(id)));
        setLoading(false);
      },
    );
  useEffect(() => {
  ...
  if (loading || !inc)
    return (
      <div className="grid place-items-center min-h-[40vh] text-ink-400">
        Loading...
      </div>
    );
  ...
  const claim = async () => {
    ...
    toast("Task claimed!", "ok");
    load();
  };
  const update = async (status) => {
    if (!myTask) return;
    await api.patch(`/api/tasks/${myTask.id}`, {
      status,
          ...
          <div>
            ...
            <p className="text-ink-500 text-sm mt-1 flex items-center gap-3 flex-wrap">
              ...
              {inc.people_affected > 0 && (
                <span className="flex items-center gap-1">
                  <Users size={14} /> ~{inc.people_affected} people affected
                </span>
              )}
              {inc.address && (
                <span className="flex items-center gap-1">
                  <MapPin size={14} /> {inc.address}
                </span>
              )}
            </p>
          </div>
      ...
      <div className="grid lg:grid-cols-[1.5fr_1fr] gap-5">
        <div className="space-y-5">
          {inc.ai_summary && (
            <Card className="!border-l-4 !border-l-purple-500">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-2 mt-4">
                <AIStat
                  label="Severity"
                  value={inc.ai_severity?.toUpperCase()}
                  color="red"
                />
                <AIStat
                  label="Confidence"
                  value={`${Math.round((inc.ai_confidence || 0) * 100)}%`}
                  color="blue"
                />
                <AIStat
                  label="ETA"
                  value={`${inc.ai_response_time_min}m`}
                  color="amber"
                />
                <AIStat
                  label="Est. victims"
                  value={inc.ai_estimated_victims}
                  color="purple"
                />
              </div>
              {inc.ai_required_resources?.length > 0 && (
                <div className="mt-4">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {inc.ai_required_resources.map((r) => (
          ...
          <Card>
            <h3 className="font-bold mb-3 flex items-center gap-2">
              <FileText size={18} /> Original Report
            </h3>
            ...
            {inc.contact_number && (
              <p className="text-sm text-ink-500 mt-3">
                📞 Contact: {inc.contact_number}
              </p>
            )}
            {inc.photo_urls?.length > 0 && (
              <div className="mt-4">
                <div className="text-xs font-bold text-ink-500 mb-2 flex items-center gap-1">
                  <ImageIcon size={14} /> Photos ({inc.photo_urls.length})
                </div>
                <div className="grid grid-cols-3 gap-2">
                  {inc.photo_urls.map((u, i) => (
                    <div
                      key={i}
                      className="aspect-square rounded-lg bg-gradient-to-br from-ink-200 to-ink-300 dark:from-ink-800 dark:to-ink-700 grid place-items-center text-ink-400"
                    >
                      <ImageIcon size={32} />
                    </div>
                  ))}
                ...
                {inc.ai_image_findings && (
                  <div className="mt-3 p-3 rounded-lg bg-ink-50 dark:bg-ink-800/60">
                    <div className="text-xs font-bold text-purple-600 mb-1 flex items-center gap-1">
                      <Sparkles size={12} /> AI Image Analysis
                    </div>
                    <ul className="text-sm list-disc list-inside text-ink-600 dark:text-ink-300">
                      {inc.ai_image_findings.map((f, i) => (
                        <li key={i}>{f}</li>
                      ))}
                    </ul>
                  </div>
        ...
        <div className="space-y-5">
          <Card>
            ...
            <div className="relative pl-6 space-y-4">
              ...
              {inc.history.map((h) => (
                <TimelineItem
                  key={h.id}
                  label={`${h.from_status ? h.from_status.replace("_", " ") + " → " : ""}${h.to_status.replace("_", " ")}`}
                  note={h.note}
                  time={h.created_at}
                  active
                />
          ...
          {user.role === "volunteer" && (
            <Card>
              <h3 className="font-bold mb-3 flex items-center gap-2">
                <Shield size={18} /> Volunteer Actions
              </h3>
              {!myTask && openTask ? (
                ...
                <>
                  <div className="text-sm mb-3">
                    ...
                    <b className="capitalize">
                      {myTask.status.replace("_", " ")}
                    </b>
                  </div>
                  <textarea
                    className="w-full rounded-xl bg-white/70 dark:bg-ink-800/70 border border-ink-200 dark:border-ink-700 p-3 text-sm mb-3"
                    rows={3}
                  ...
                  <div className="flex flex-wrap gap-2">
                    {myTask.status !== "en_route" &&
                      myTask.status !== "completed" && (
                        <Button size="sm" onClick={() => update("en_route")}>
                          🚗 En Route
                        </Button>
                      )}
                    {myTask.status !== "on_site" &&
                      myTask.status !== "completed" && (
                        <Button
                          size="sm"
                          variant="warning"
                          onClick={() => update("on_site")}
                        >
                          📍 On Scene
                        </Button>
                      )}
                  ...
                  {myTask.notes && (
                    <div className="mt-3 p-3 rounded-lg bg-ink-50 dark:bg-ink-800/60 text-sm whitespace-pre-wrap">
                      {myTask.notes}
                    </div>
                  )}
                </>
              ...
              <div className="grid grid-cols-2 gap-2">
                {[
                  ...
                  <Button
                    ...
                    variant="ghost"
                    onClick={async () => {
                      await api.patch(`/api/incidents/${id}`, { status: s });
                      load();
                    }}
...
function TimelineItem({ label, note, time, active }) {
  return (
    <div className="relative">
      <div
        className={`absolute -left-[22px] top-1 w-4 h-4 rounded-full border-2 ${active ? "bg-brand-500 border-brand-600" : "bg-ink-300 dark:bg-ink-600 border-ink-200 dark:border-ink-500"}`}
      />
