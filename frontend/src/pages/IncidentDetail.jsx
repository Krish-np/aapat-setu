  ...
  return (
    <motion.div initial={{ opacity:0, y:8 }} animate={{ opacity:1, y:0 }} transition={{ duration:.3 }} className="space-y-5">
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="min-w-0">
            ...
            <p className="text-ink-500 dark:text-ink-400 text-sm mt-1 flex items-center gap-3 flex-wrap">
              ...
              {(inc.people_affected||0)>0 && <span className="flex items-center gap-1"><Users size={14}/> ~{inc.people_affected} {t('incident.people_affected')}</span>}
          ...
          <div className="flex gap-2">
            {isResponder && inc.status !== 'resolved' && (
              <Button size="sm" variant="success" onClick={async () => { await api.patch(`/api/incidents/${id}`,{status:'resolved'}); toast('Resolved','ok'); load() }}>
                <CheckCircle2 size={16}/> {t('incident.mark_resolved')}
              </Button>
            )}
          </div>
        </div>
      ...
      <div className="grid lg:grid-cols-[1.5fr_1fr] gap-5">
        <div className="space-y-5">
          {inc.ai_summary && (
            <Card className="!border-l-4 !border-l-purple-500">
              <div className="flex items-center gap-2 mb-2"><Sparkles className="text-purple-500" size={18}/> <span className="text-xs font-bold uppercase tracking-wider text-purple-600 dark:text-purple-400">{t('incident.ai_briefing')}</span></div>
              <p className="text-base font-semibold leading-relaxed">{inc.ai_summary}</p>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-2 mt-4">
                <AIStat label={t('incident.severity')} value={inc.ai_severity?.toUpperCase()} color="red"/>
                <AIStat label={t('incident.confidence')} value={`${Math.round((inc.ai_confidence||0)*100)}%`} color="blue"/>
                <AIStat label={t('common.eta')} value={`${inc.ai_response_time_min}m`} color="amber"/>
                <AIStat label={t('incident.est_victims')} value={inc.ai_estimated_victims} color="purple"/>
              </div>
              {inc.ai_required_resources?.length>0 && (
                <div className="mt-4">
                  <div className="text-xs font-bold text-ink-500 mb-2">{t('incident.required_resources')}</div>
              ...
              {inc.ai_safety_instructions && (
                <div className="mt-4 p-3 rounded-xl bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/30 text-sm">
                  <div className="text-xs font-bold text-amber-700 dark:text-amber-300 mb-1">⚠️ {t('incident.safety_instructions')}</div>
                  {inc.ai_safety_instructions}
                </div>
              )}
            </Card>
          ...
          <Card className="!p-0 overflow-hidden">
            <div className="p-5 border-b border-ink-200 dark:border-ink-800 font-bold flex items-center justify-between">
              <div className="flex items-center gap-2"><MapPin size={18}/> {t('common.location')}</div>
              <button
                onClick={() => {
                  if (!navigator.geolocation) return
                  setLocStatus('requesting')
              ...
              >
                {locStatus === 'requesting' ? t('incident.locating') : locStatus === 'denied' ? t('incident.loc_blocked') : myLoc ? t('incident.recenter_me') : t('incident.use_my_location')}
              </button>
            </div>
            <Map
              incidents={[inc]}
          ...
          <Card>
            <h3 className="font-bold mb-3 flex items-center gap-2"><FileText size={18}/> {t('incident.original_report')}</h3>
            <p className="leading-relaxed whitespace-pre-wrap">{inc.description}</p>
            {inc.contact_number && <p className="text-sm text-ink-500 mt-3">📞 {t('incident.contact')}: {inc.contact_number}</p>}
            {inc.photo_urls?.length>0 && (
              <div className="mt-4">
                <div className="text-xs font-bold text-ink-500 mb-2 flex items-center gap-1"><ImageIcon size={14}/> {t('incident.photos')} ({inc.photo_urls.length})</div>
                ...
                {inc.ai_image_findings && (
                  <div className="mt-3 p-3 rounded-lg bg-ink-50 dark:bg-ink-800/60">
                    <div className="text-xs font-bold text-purple-600 mb-1 flex items-center gap-1"><Sparkles size={12}/> {t('incident.ai_image_analysis')}</div>
        ...
        <div className="space-y-5">
          <Card>
            <h3 className="font-bold mb-4 flex items-center gap-2"><Clock size={18}/> {t('incident.status_timeline')}</h3>
            <div className="relative pl-6 space-y-4">
              <div className="absolute left-2 top-2 bottom-2 w-0.5 bg-ink-200 dark:bg-ink-700"/>
              <TimelineItem label={t('incident.report_submitted')} time={inc.created_at} active/>
          ...
          {user.role === 'volunteer' && (
            <Card>
              <h3 className="font-bold mb-3 flex items-center gap-2"><Shield size={18}/> {t('incident.volunteer_actions')}</h3>
              {!myTask && openTask ? (
                <Button className="w-full" onClick={claim}>{t('incident.claim_mission')}</Button>
              ) : myTask ? (
                <>
                  <div className="text-sm mb-3">{t('common.status')}: <b className="capitalize">{myTask.status.replace('_',' ')}</b></div>
                  <textarea className="w-full rounded-xl bg-white/70 dark:bg-ink-800/70 border border-ink-200 dark:border-ink-700 p-3 text-sm mb-3" rows={3} placeholder={t('incident.share_update')} value={note} onChange={e=>setNote(e.target.value)}/>
                  <div className="flex flex-wrap gap-2">
                    {myTask.status !== 'en_route' && myTask.status !== 'completed' && <Button size="sm" onClick={()=>update('en_route')}>{t('incident.en_route')}</Button>}
                    {myTask.status !== 'on_site' && myTask.status !== 'completed' && <Button size="sm" variant="warning" onClick={()=>update('on_site')}>{t('incident.on_scene')}</Button>}
                    {myTask.status !== 'in_progress' && myTask.status !== 'completed' && <Button size="sm" variant="secondary" onClick={()=>update('in_progress')}>{t('incident.rescue_ongoing')}</Button>}
              ...
              ) : <div className="text-sm text-ink-500">{t('incident.no_open_tasks', 'No open tasks.')}</div>}
            </Card>
          )}

          {isResponder && (
            <Card>
              <h3 className="font-bold mb-3">{t('incident.response_actions')}</h3>
...
function ReportedTime({ time }) {
  ...
  const { t } = useTranslation()
  return <span>{t('incident.reported_at', { when })}</span>
}
