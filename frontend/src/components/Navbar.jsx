  ...
  return (
    <nav className="sticky top-0 z-50 glass-strong border-b border-ink-200/60 dark:border-ink-800/60">
      <div className="max-w-7xl mx-auto px-4 md:px-6 h-16 flex items-center gap-4">
        <Link
          to="/"
          className="flex items-center gap-2.5 font-extrabold text-lg shrink-0"
        >
        ...
        <div className="hidden md:flex items-center gap-1 ml-4">
          <a
            href="#features"
            className="h-9 px-3 inline-flex items-center gap-1.5 text-sm font-medium text-ink-600 dark:text-ink-300 hover:text-ink-900 dark:hover:text-white rounded-lg hover:bg-ink-100 dark:hover:bg-ink-800 transition"
          >
        ...
        <div className="ml-auto flex items-center gap-1.5">
          ...
          <button
            ...
            title={
              i18n.language === "en"
                ? "Switch to Nepali / नेपालीमा स्विच"
                : "Switch to English"
            }
            className="h-9 rounded-xl bg-ink-100 dark:bg-ink-800 p-1 flex items-center text-[11px] font-bold transition hover:bg-ink-200 dark:hover:bg-ink-700"
          >
            <span
              className={`px-2 py-1 rounded-lg transition ${i18n.language !== "ne" ? "bg-white dark:bg-ink-950 shadow-sm text-brand-600 dark:text-brand-300" : "text-ink-500"}`}
            >
              EN
            </span>
            <span
      ...
      {open && (
        <div className="md:hidden border-t border-ink-200/60 dark:border-ink-800 bg-white/95 dark:bg-ink-950/95 backdrop-blur px-4 py-3 flex flex-col gap-1">
          <a
            ...
            className="px-3 py-2 rounded-lg text-sm text-ink-700 dark:text-ink-200 hover:bg-ink-100 dark:hover:bg-ink-800 flex items-center gap-2"
          >
            <Sparkles size={15} /> Features
          </a>
          <a
