...
export function Toaster() {
  ...
  const push = useCallback((msg, kind = "ok") => {
    ...
    if (_recent.size > 200) {
      for (const [k, ts] of _recent) {
        if (now - ts > 10000) _recent.delete(k);
      }
    }
  ...
  const icons = {
    ...
    loading: (
      <Loader2
        className="text-brand-600 flex-shrink-0 animate-spin"
        size={20}
      />
    ),
  };
  const styles = {
    ...
    success:
      "border-emerald-500/40 bg-emerald-50/95 dark:bg-emerald-500/10 dark:border-emerald-500/30 text-emerald-900 dark:text-emerald-200",
    err: "border-red-500/40 bg-red-50/95 dark:bg-red-500/10 dark:border-red-500/30 text-red-900 dark:text-red-200",
    error:
      "border-red-500/40 bg-red-50/95 dark:bg-red-500/10 dark:border-red-500/30 text-red-900 dark:text-red-200",
    alert:
      "border-amber-500/40 bg-amber-50/95 dark:bg-amber-500/10 dark:border-amber-500/30 text-amber-900 dark:text-amber-200",
    ...
    loading:
      "border-brand-500/40 bg-white/95 dark:bg-ink-900/95 dark:border-brand-500/30 text-ink-900 dark:text-ink-100",
  };
